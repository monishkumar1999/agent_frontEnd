...
const DurationAgreement = () => {
    ...
    const fetchDescribes = async () => {
        ...
        try {
            const response = await axios.get(`${server}/durationRoute/view`, {
    ...
    const handleAddDescribe = async (e) => {
        ...
        try {
            const response = await axios.post(
                `${server}/durationRoute/add`,
                { name: newDescribeName },
                {
                    withCredentials: true,
                }
            );
            toast.success('Sale method added successfully!');
    ...
    return (
        <div>
            <h1 className="text-xl font-bold mb-4">Agency Service Provide List</h1>
...
export default DurationAgreement;
