...
function SignIn({ onLoginSuccess }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button
        type="submit"
        className="w-full py-3 bg-blue-500 text-white font-semibold rounded-lg flex items-center justify-center"
      >
        {loading ? (
      ...
      <GoogleLogin
        onSuccess={async (credentialResponse) => {
          ...
          await axiosInstance.post("user/google-login", { email });
        }}
        onError={() => {
